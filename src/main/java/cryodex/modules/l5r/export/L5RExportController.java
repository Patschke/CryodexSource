...
public class L5RExportController extends ExportController {
	...
	public String appendRankings(Tournament tournament) {
		...
		Collections.sort(playerList, new L5RComparator(tournament, L5RComparator.rankingCompare));
		...
		while (pageCounter <= increment) {
			for (; index < matches.size(); index = index + increment) {
				...
				if (m.getPlayer2() != null) {
					...
					L5RPlayer xp1 = (L5RPlayer) m.getPlayer1().getModuleInfoByModule(tournament.getModule());
					L5RPlayer xp2 = (L5RPlayer) m.getPlayer2().getModuleInfoByModule(tournament.getModule());
