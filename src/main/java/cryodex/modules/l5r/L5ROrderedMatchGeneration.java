 ...
 * 
 */
public class L5ROrderedMatchGeneration {
	...
	public L5ROrderedMatchGeneration(L5RTournament tournament,
			List<Player> players) {
	...
	public List<Match> generateMatches() {
		...
		Collections.sort(tempList, new L5RComparator(tournament,
				L5RComparator.pairingCompare));
	...
	private int getScore(List<Match> matches) {
		...
		// order players
		Collections.sort(players, new L5RComparator(tournament,
				L5RComparator.pairingCompare));

		// get list of players in order of matches
		List<L5RPlayer> playerByMatchOrder = new ArrayList<L5RPlayer>();
		for (Match xm : matches) {
			playerByMatchOrder.add((L5RPlayer) xm.getPlayer1().getModuleInfoByModule(tournament.getModule()));
			playerByMatchOrder.add((L5RPlayer) xm.getPlayer2().getModuleInfoByModule(tournament.getModule()));
		}
