...
public class SWLCGOrderedMatchGeneration {
	...
	private int getScore(List<Match> matches) {
		...
		for (Match xm : matches) {
			playerByMatchOrder.add((SWLCGPlayer) xm.getPlayer1().getModuleInfoByModule(tournament.getModule()));
			playerByMatchOrder.add((SWLCGPlayer) xm.getPlayer2().getModuleInfoByModule(tournament.getModule()));
		}
